...
docker run \
       ...
       --network none \
       --name tal-golfscript-$$ --rm \
       tal-golfscript:latest /bin/sh /home/tal/launch.sh 9
for testcount in `seq 0 1 9`
do
    ...
    cat out$testcount.txt | head -n 3 > o2$testcount.txt
    diff o2$testcount.txt input$testcount.txt || errc=1
    if [ "$errc" -ne "0" ]; then
	set +x 
