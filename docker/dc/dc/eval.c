...
typedef enum {
	...
	DC_STRRAW,		/* caller needs to parse a dc_str from input stream */
	...
	int negcmp DC_DECLEND
{
	...
	case '?':	/* read a line from standard-input and eval it */
		...
		dc_push(datum);
		return DC_EVALTOS;
	case '@':	/* read a line from standard-input and put it on stack */
		if (stdin_lookahead != EOF){
			ungetc(stdin_lookahead, stdin);
			stdin_lookahead = EOF;
		}
...
between 2 and %d (inclusive)\n",
    ...
    /* week.golf extensions */
        
	case 'T' : /* transforms a string in a sequence of chars */
        if (dc_pop(&datum) == DC_SUCCESS){
            if (datum.dc_type == DC_STRING) {
                /* right type datum.v.string */
                int sz=0;
                if (datum.v.string!=NULL) {
                    sz = datum.v.string->s_len;
                    for (int i=datum.v.string->s_len-1;i>=0;--i)
                        dc_push(dc_char2string(datum.v.string->s_ptr[i]));
                }
                dc_push(dc_int2data(sz));
            } else {
                // just push 0
                dc_push(dc_int2data(0));
            }
        }
        //return DC_EATONE;
        break;
	case 'U' : /* Convert top of stack to ascii code. */
		if (dc_pop(&datum) == DC_SUCCESS){
			char tmps;
			if (datum.dc_type == DC_NUMBER){
				tmps = (char) dc_num2int(datum.v.number, DC_TOSS);
